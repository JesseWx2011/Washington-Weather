...
// Version 2.06a - 02-Apr-2018 - adapted for Weather-Display dailynoaareport*.htm display (by someone who didn't take credit)
// Version 2.07 - 14-May-2021 - added WeeWX support
// Version 2.08 - 27-Dec-2022 - fixes for PHP 8.2
// Version 2.09 - 17-Feb-2023 - Merged in Weather-Display code to base
// Version 2.10 - 18-Feb-2023 - fixed Deprecated errata messages after WD merge

$NOAAversion = "include-noaa-reports.php - Version 2.10 - 18-Feb-2023";
...
$NOAAdir = isset($SITE['NOAAdir'])?$SITE['NOAAdir']:'./';
$NOAACurDir = isset($SITE['NOAACurDir'])?$SITE['NOAACurDir']:'./';
$Naming = isset($SITE['WXsoftware'])?$SITE['WXsoftware']:'Unk';  // from the [$SITE['WXsoftware']]-defs.php file
...
if ($Naming == 'WV' or $Naming == 'WEEWX' or $Naming == 'WD') {
  $validWX = true;
}
...
if ($Naming == 'WD') { // special for Weather-Display files
  $now_month_nz = $now_month + 0;  // remove leading zero if any
	$ThisYearFile = $NOAAdir."noaareportyear{$now_year}.htm";
	$ThisMonthFile= $NOAAdir."dailynoaareport{$now_month_nz}{$now_year}.htm";
}
...
//  echo "<pre style=\"font-size: 9pt;\">\n";
  ...
  foreach ($files as $key => $file) {
	if ($Naming == 'WL') {
       ...
       $year = substr($file,0,4);
	} elseif ($Naming == 'WV' or $Naming == 'WEEWX') {
		...
		if (substr($file, 4, 2) == "MO") { // found a month report
		  ...
		  preg_match('!dailynoaareport.*(\d{4}).htm!',$file,$matches);
			if(isset($matches[1])) {
		    $year = $matches[1];
			}
	}
   if($doDebug) { print "<!-- file='$file' year='$year' -->\n"; }
   if (!isset($year) or ! preg_match("/^\d{4}$/",$year) ) { continue;} //make sure year is numeric
...
//
 ...
 for ($y = $now_year; $y >= $first_year; $y--) {
   ...
   if ($Naming == 'WV' or $Naming == 'WEEWX')  {$t = "NOAA-$yy.txt"; }
   if ($Naming == 'WD')  {$t = "noaareportyear$yy.htm"; }   
   ...
   for ($m = 1; $m <= 12; $m++) {
	 ...
	 if ($Naming == 'WV' or $Naming == 'WEEWX') {$testfile = "NOAA-$yy-$mm.txt"; }
	 if ($Naming == 'WD') {$testfile = "dailynoaareport$m$yy.htm"; }	 
...
$WDSMALL = ($Naming == 'WD')?' font-size: 10px;margin-left: -8px;padding-left: 5px;':'';

$BIDI = $RTLlang?' style="unicode-bidi: bidi-override;direction: ltr;"':'';
if($BIDI == '' and $WDSMALL !== '') {$BIDI = ' style="'.$WDSMALL.'"';}
 if (! $yr && ! $mo && $Naming != "CU" && $Naming != "WV" && $Naming != 'WEEWX') {  // special for 'current month'
    echo "<br /><b>".langtransstr('Report for')." $now_year ".$longmonths[$now_month]."</b>\n";
    $rpt = implode("",file("$ThisMonthFile"));
	  if($Naming == 'WD') { $rpt = fixup_WD_report($rpt); }
	  echo "<br />\n<pre$BIDI>\n";
	  $rpt = preg_replace('|<|Uis','&lt;',$rpt);
	  $rpt = preg_replace('|院Uis','&deg;',$rpt);
	  echo $rpt;
  }
 if (! $yr && ! $mo && ($Naming == "WV" or $Naming == 'WEEWX')) {  // special for 'current month' and wview/weewx
   ...
   if ($yr) {
    if ($mo) { // month given
	    if ($Naming == 'WL')  {$testfile = "NOAA$yr-$mo.TXT";}
		  ...
		  if ($Naming == 'WV' or $Naming == 'WEEWX')  {$testfile = "NOAA-$yr-$mo.txt";}
		  if ($Naming == 'WD')  {$mnz = $mo+0;$testfile = "dailynoaareport$mnz$yr.htm";}

	  } else { // no month given
	    if ($Naming == 'WL')  {$testfile = "NOAA$yr.TXT";}
		  if ($Naming == 'VWS') {$testfile = "$yr.txt";}
		  if ($Naming == 'CU')  {$testfile = "NOAAYR$yr.txt";}
		  if ($Naming == 'WV' or $Naming == 'WEEWX')  {$testfile = "NOAA-$yr.txt";}
		  if ($Naming == 'WD')  {$testfile = "noaareportyear$yr.htm";}
	  } // no month given
     if (isset($filesfound[$testfile])) {
...
function fixup_WD_report($inrep) {
	$r = str_replace('>=','&gt;',$inrep);
	$r = str_replace('<=','&lt;',$r);
	$r = strip_tags($r); // get rid of html embedded
  
	//$r = preg_replace('!     !is',' ',$r); // Prune back the spacing
	//$r = preg_replace('!    !is',' ',$r); // prune back the spacing more
	return($r);
